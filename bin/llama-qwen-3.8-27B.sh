#!/bin/bash -e
# Qwen3.8-27B (unsloth UD-Q4_K_XL). The qwen3.8-27B.sh from the llama.cpp
# fork, with paths that make sense inside the container: logs under
# ./runs/llama, the model from the mounted Hugging Face cache (no re-download).
# Started by llama-server.sh when config/settings.yaml says llm.model: Qwen3.8-27B.
#
