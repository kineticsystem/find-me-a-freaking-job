...
- **On the host, bind-mounted:** `config/`, `data/`, `runs/` — everything you edit and everything the app persists — and `modules/llama.cpp`, the llama.cpp fork with its build tree. Survives rebuilds, restarts and `clean`. `config/settings.yaml` is created from its `.example` on first start and is git-ignored; everything personal is in `data/jobs.db`.
