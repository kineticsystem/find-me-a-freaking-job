...
The model is whatever llama.cpp serves; the wrapper only assumes an OpenAI-compatible endpoint and tool calling. Development used Qwen 3.8 27B on an RTX 4090 (24 GB), where generation runs at ~95 tok/s; the shipped default is now NVIDIA Ornith-1.5-35B-A3B, chosen on the measurements in `score_comparison.md` (same ranking as Qwen, ~1.5× faster per batch, a scale on which the deep dive is reachable). What makes a session slow there is not generation but the model's thinking, which Qwen 3 leaves unbounded by default (`--reasoning-budget -1`). Extraction sessions, which produce a lot of JSON, take two to four minutes; triage and deep dive about one. A different model or card shifts all of these numbers.
...
| `user_state` | A user's decisions on a job, keyed `(job_id, user_id)`: `shortlisted`, `applied`, `declined` (applied, then turned down), `dismissed` (with a reason), `archived`, plus notes; no row means `new`. Separate from `evaluations` on purpose — a re-run never touches it. |
...
**Criteria hash.** Every evaluation is stamped with `sha256(cv_and_notes_hash | preferences_hash | model_name)`. A job "needs evaluation" when it has no row for the current hash. So editing the CV, the notes or `preferences.yaml` automatically makes every job eligible for re-scoring on the next run, and the old scores remain queryable.
